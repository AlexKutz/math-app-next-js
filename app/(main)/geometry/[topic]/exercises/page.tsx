...
export default async function Page({
  ...
  const tasksDir = join(process.cwd(), 'content/geometry', topic, 'tasks');
  const lessonPath = join(process.cwd(), 'content/geometry', topic);

  const tasks = loadTasks(tasksDir);
  const { frontmatter } = LoadLesson(lessonPath);

  const breadcrumbItems = [
    { label: 'Геометрія', href: '/geometry' },
    { label: frontmatter.title || topic, href: `/geometry/${topic}/lesson` },
    { label: 'Вправи' },
  ];
